    ...
    <section id="testimonials" className="py-32 md:py-40 bg-[var(--color-navy)] relative overflow-hidden">
      ...
      <div className="max-w-7xl mx-auto px-8 lg:px-12 relative z-10">
        <div className="text-center mb-24">
          ...
          <motion.h2 initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.6, delay: 0.1 }} className="font-serif text-5xl md:text-6xl font-bold text-gradient-cream">{t.testimonials.title}</motion.h2>
        </div>
        <div className="grid md:grid-cols-3 gap-8">
          {t.testimonials.items.map((item, i) => (
            <motion.div key={item.author} initial={{ opacity: 0, y: 40 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true, margin: "-100px" }} transition={{ duration: 0.6, delay: i * 0.15 }} className="glass-panel p-10 rounded-2xl border hover:border-gold/30 hover:shadow-[0_0_30px_rgba(212,175,55,0.15)] hover:-translate-y-2 transition-all duration-500 relative group">
              ...
              <p className="font-serif text-cream/90 text-xl leading-relaxed mb-10 italic">
                <span className="text-transparent bg-clip-text bg-gradient-to-b from-gold to-gold/30 text-5xl not-italic leading-[0] absolute -top-2 -left-2 tracking-tighter opacity-70">&ldquo;</span>
                <span className="relative z-10">{item.quote}</span>
              </p>
              <div className="pt-6 border-t border-white/10 flex items-center justify-between">
                <div>
                  <div className="font-bold text-cream tracking-wide">{item.author}</div>
                  <div className="text-gold/80 text-sm mt-1 uppercase tracking-widest">{item.location}</div>
                </div>
              </div>
            </motion.div>
          ))}
