    ...
    <section id="services" className="py-32 md:py-40 bg-dark relative overflow-hidden">
      ...
      <div className="max-w-6xl mx-auto px-8 lg:px-12 relative z-10">
        <div className="text-center mb-20 flex flex-col items-center">
          <motion.span initial={{ opacity: 0, y: 16 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true, margin: "-100px" }} transition={{ duration: 0.6 }} className="text-gold text-sm tracking-[0.4em] uppercase font-semibold mb-4">{t.services.tag}</motion.span>
          <motion.h2 initial={{ opacity: 0, y: 16 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true, margin: "-100px" }} transition={{ duration: 0.6, delay: 0.1 }} className="font-serif text-5xl md:text-6xl font-bold text-gradient-cream">{t.services.title}</motion.h2>
          <motion.p initial={{ opacity: 0, y: 16 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true, margin: "-100px" }} transition={{ duration: 0.6, delay: 0.2 }} className="text-cream/50 text-lg mt-6 max-w-xl mx-auto leading-relaxed">{t.services.desc}</motion.p>
        </div>

        <div className="grid md:grid-cols-3 gap-8">
          {t.services.items.map((service, i) => (
            <motion.div key={service.title} initial={{ opacity: 0, y: 40 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true, margin: "-100px" }} transition={{ duration: 0.7, delay: i * 0.15 + 0.2 }} whileHover={{ y: -8 }} className="group flex flex-col rounded-xl glass-panel glow-border p-10 h-full">
              <div className="text-gold mb-8 opacity-90 group-hover:scale-110 group-hover:opacity-100 transition-all duration-500 drop-shadow-[0_0_15px_rgba(212,175,55,0.4)]">{icons[i]}</div>
              <h3 className="font-serif text-2xl font-bold mb-4 text-cream">{service.title}</h3>
              <p className="text-cream/60 text-base leading-relaxed mb-8 flex-1">{service.description}</p>
